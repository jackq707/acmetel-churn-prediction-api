...
TH = dict(plot_bgcolor="#0f172a", paper_bgcolor="rgba(0,0,0,0)", font=dict(family="Inter,sans-serif", size=13, color="#e2e8f0"), margin=dict(t=20,b=35,l=45,r=20))
...
with col1:
    ...
    fig.update_layout(height=300, showlegend=False, yaxis=dict(ticksuffix="%",gridcolor="#1e293b",zeroline=False,tickfont=dict(size=13)), xaxis=dict(gridcolor="#1e293b",tickfont=dict(size=13),tickangle=0,tickformat="%b %d"), margin=dict(t=20,b=35,l=45,r=20), plot_bgcolor="#0f172a", paper_bgcolor="rgba(0,0,0,0)", font=dict(family="Inter,sans-serif",size=13,color="#e2e8f0"))
...
with col2:
    ...
    fig2.update_layout(height=250, showlegend=False, margin=dict(t=20,b=10,l=0,r=0), paper_bgcolor="rgba(0,0,0,0)")
...
with col3:
    ...
    if frames:
        ...
        fig3.update_layout(height=300, showlegend=False, xaxis=dict(ticksuffix="%",gridcolor="#1e293b",range=[0,105],tickfont=dict(size=13)), yaxis=dict(tickfont=dict(size=13)), **TH)
...
with col_l:
    ...
    if "tenure" in df.columns and df["tenure"].notna().any():
        ...
        fig4.update_layout(barmode="overlay", height=300, legend=dict(orientation="h",y=0.98,x=0.02,font=dict(size=13),bgcolor="rgba(15,23,42,0.8)",bordercolor="rgba(0,0,0,0)"), xaxis=dict(title="Tenure (months)",gridcolor="#1e293b",tickfont=dict(size=13)), yaxis=dict(gridcolor="#1e293b",tickfont=dict(size=13)), **TH)
