...
.sh{font-size:12px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px;padding-bottom:3px;border-bottom:2px solid #1e293b}
...
with c1:
    ...
    st.markdown(f"""
    <div style="background:#1e293b;border-radius:8px;padding:16px 20px;display:flex;align-items:center;justify-content:space-between;min-height:72px">
        ...
        <a href="data:text/csv;base64,{b64}" download="acmetel_batch_template.csv"
           style="background:#1e293b;color:#e2e8f0;border:1px solid #334155;border-radius:6px;padding:6px 18px;font-size:14px;text-decoration:none;white-space:nowrap;font-family:sans-serif">
            Download
        </a>
    </div>
    """, unsafe_allow_html=True)
...
with c3:
    ...
    if uploaded:
        ...
        st.markdown("""
        <div style="background:#1e293b;border-radius:8px;padding:16px 20px;min-height:72px;display:flex;align-items:center;gap:12px">
            <span style="font-size:1.6rem">✅</span>
            <div>
...
st.markdown('<hr style="border:1px solid #1e293b;margin:6px 0">', unsafe_allow_html=True)
...
if uploaded:
        ...
        with st.spinner(f"Predicting {len(records)} customers..."):
            try:
                ...
                with cs:
                    ...
                    c2b.markdown(f'<div class="sb" style="background:#1e293b;color:#e2e8f0">Avg Prob<br><b>{avgp:.3f}</b></div>', unsafe_allow_html=True)
    ...
    st.markdown("""
    <div style="background:#1e293b;border:2px dashed #334155;border-radius:12px;padding:30px;text-align:center">
